...
public interface MultiLineLabel {
    ...
    static MultiLineLabel create(Font p_94342_, FormattedText p_94343_, int p_94344_) {
        return createFixed(
            p_94342_,
            p_94342_.split(p_94343_, p_94344_)
                .stream()
                .map(p_94374_ -> new MultiLineLabel.TextWithWidth(p_94374_, p_94342_.width(p_94374_)))
    ...
    static MultiLineLabel create(Font p_94346_, FormattedText p_94347_, int p_94348_, int p_94349_) {
        return createFixed(
            p_94346_,
            p_94346_.split(p_94347_, p_94348_)
                .stream()
                .limit((long)p_94349_)
                .map(p_94371_ -> new MultiLineLabel.TextWithWidth(p_94371_, p_94346_.width(p_94371_)))
                .collect(ImmutableList.toImmutableList())
        );
    }
    ...
    static MultiLineLabel create(Font p_94351_, Component... p_94352_) {
        return createFixed(
            p_94351_,
            Arrays.stream(p_94352_)
                .map(Component::getVisualOrderText)
                .map(p_94360_ -> new MultiLineLabel.TextWithWidth(p_94360_, p_94351_.width(p_94360_)))
                .collect(ImmutableList.toImmutableList())
        );
    }
    ...
    static MultiLineLabel create(Font p_169037_, List<Component> p_169038_) {
        return createFixed(
            ...
            p_169038_.stream()
                .map(Component::getVisualOrderText)
                .map(p_169035_ -> new MultiLineLabel.TextWithWidth(p_169035_, p_169037_.width(p_169035_)))
                .collect(ImmutableList.toImmutableList())
        );
    }

    static MultiLineLabel createFixed(final Font p_94362_, final List<MultiLineLabel.TextWithWidth> p_94363_) {
        return p_94363_.isEmpty()
            ? EMPTY
            : new MultiLineLabel() {
                private final int width = p_94363_.stream().mapToInt(p_232527_ -> p_232527_.width).max().orElse(0);
                ...
                public int renderCentered(GuiGraphics p_281603_, int p_281267_, int p_281819_, int p_281545_, int p_282780_) {
                    ...
                    for (MultiLineLabel.TextWithWidth multilinelabel$textwithwidth : p_94363_) {
                        p_281603_.drawString(
                            p_94362_, multilinelabel$textwithwidth.text, p_281267_ - multilinelabel$textwithwidth.width / 2, i, p_282780_
                        );
                        i += p_281545_;
                    }
                ...
                public void renderBackgroundCentered(GuiGraphics p_281633_, int p_210832_, int p_210833_, int p_210834_, int p_210835_, int p_210836_) {
                    int i = p_94363_.stream().mapToInt(p_232524_ -> p_232524_.width).max().orElse(0);
                    if (i > 0) {
                        p_281633_.fill(
                            p_210832_ - i / 2 - p_210835_,
                            p_210833_ - p_210835_,
                            p_210832_ + i / 2 + p_210835_,
                            p_210833_ + p_94363_.size() * p_210834_ + p_210835_,
                            p_210836_
                        );
                    }
                }
