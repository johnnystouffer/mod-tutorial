...
public class ChunkMap extends ChunkStorage implements ChunkHolder.PlayerProvider {
    ...
    private CompletableFuture<ChunkResult<List<ChunkAccess>>> getChunkRangeFuture(ChunkHolder p_281446_, int p_282030_, IntFunction<ChunkStatus> p_282923_) {
        if (p_282030_ == 0) {
            ...
            return p_281446_.getOrScheduleFuture(chunkstatus1, this).thenApply(p_326378_ -> p_326378_.map(List::of));
        } else {
    ...
    public CompletableFuture<ChunkResult<ChunkAccess>> schedule(ChunkHolder p_140293_, ChunkStatus p_333048_) {
        ChunkPos chunkpos = p_140293_.getPos();
        if (p_333048_ == ChunkStatus.EMPTY) {
            return this.scheduleChunkLoad(chunkpos).thenApply(ChunkResult::of);
        } else {
            if (p_333048_ == ChunkStatus.LIGHT) {
                this.distanceManager.addTicket(TicketType.LIGHT, chunkpos, ChunkLevel.byStatus(ChunkStatus.LIGHT), chunkpos);
            }

            if (!p_333048_.hasLoadDependencies()) {
                ChunkAccess chunkaccess = p_140293_.getOrScheduleFuture(p_333048_.getParent(), this).getNow(ChunkHolder.UNLOADED_CHUNK).orElse(null);
                if (chunkaccess != null && chunkaccess.getStatus().isOrAfter(p_333048_)) {
                    CompletableFuture<ChunkAccess> completablefuture = p_333048_.load(
                        this.worldGenContext, p_326377_ -> this.protoChunkToFullChunk(p_140293_, p_326377_), chunkaccess
                    );
            ...
            return this.scheduleChunkGeneration(p_140293_, p_333048_);
        }
    }
    ...
    private CompletableFuture<ChunkAccess> scheduleChunkLoad(ChunkPos p_140418_) {
        return this.readChunk(p_140418_).thenApply(p_214925_ -> p_214925_.filter(p_214928_ -> {
            ...
            if (p_326416_.isPresent()) {
                ChunkAccess chunkaccess = ChunkSerializer.read(this.level, this.poiManager, p_140418_, p_326416_.get());
    ...
    private CompletableFuture<ChunkResult<ChunkAccess>> scheduleChunkGeneration(ChunkHolder p_140361_, ChunkStatus p_329310_) {
        ...
        CompletableFuture<ChunkResult<List<ChunkAccess>>> completablefuture = this.getChunkRangeFuture(
            p_140361_, p_329310_.getRange(), p_326403_ -> this.getDependencyStatus(p_329310_, p_326403_)
        );
        ...
        return completablefuture.thenComposeAsync(p_326401_ -> {
            ...
            if (list == null) {
                ...
                try {
                    ...
                    if (chunkaccess.getStatus().isOrAfter(p_329310_)) {
                        completablefuture1 = p_329310_.load(this.worldGenContext, p_326386_ -> this.protoChunkToFullChunk(p_140361_, p_326386_), chunkaccess);
                    } else {
                        completablefuture1 = p_329310_.generate(this.worldGenContext, executor, p_326405_ -> this.protoChunkToFullChunk(p_140361_, p_326405_), list);
                    }
    ...
    private ChunkStatus getDependencyStatus(ChunkStatus p_332383_, int p_140264_) {
        ...
        if (p_140264_ == 0) {
            chunkstatus = p_332383_.getParent();
        } else {
            chunkstatus = ChunkStatus.getStatusAroundFullChunk(ChunkStatus.getDistance(p_332383_) + p_140264_);
        }
