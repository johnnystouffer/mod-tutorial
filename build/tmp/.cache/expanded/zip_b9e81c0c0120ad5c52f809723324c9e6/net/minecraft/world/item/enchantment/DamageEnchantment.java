...
public class DamageEnchantment extends Enchantment {
    ...
    public DamageEnchantment(Enchantment.EnchantmentDefinition p_330513_, Optional<TagKey<EntityType<?>>> p_332960_) {
        super(p_330513_);
        this.targets = p_332960_;
    }

    @Override
    public float getDamageBonus(int p_44635_, @Nullable EntityType<?> p_331449_) {
        if (this.targets.isEmpty()) {
            return 1.0F + (float)Math.max(0, p_44635_ - 1) * 0.5F;
        } else {
            return p_331449_ != null && p_331449_.is(this.targets.get()) ? (float)p_44635_ * 2.5F : 0.0F;
        }
    }
