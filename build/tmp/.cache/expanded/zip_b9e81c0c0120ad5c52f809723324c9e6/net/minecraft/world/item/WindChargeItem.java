...
public class WindChargeItem extends Item implements ProjectileItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level p_333953_, Player p_328676_, InteractionHand p_332155_) {
        if (!p_333953_.isClientSide()) {
            Vec3 vec3 = p_328676_.getEyePosition().add(p_328676_.getForward().scale(0.8F));
            if (!p_333953_.getBlockState(BlockPos.containing(vec3)).canBeReplaced()) {
                vec3 = p_328676_.getEyePosition().add(p_328676_.getForward().scale(0.05F));
            }

            WindCharge windcharge = new WindCharge(p_328676_, p_333953_, vec3.x(), vec3.y(), vec3.z());
    ...
    public Projectile asProjectile(Level p_335187_, Position p_334268_, ItemStack p_330980_, Direction p_331337_) {
        ...
        return new WindCharge(p_335187_, p_334268_.x(), p_334268_.y(), p_334268_.z(), d0, d1, d2);
    }
